...
const EN: Dict = {
  ...
  tx_episodes_line: '{n} episodes · autonomy {a}% · style {s} · went in circles {sp}',
  ...
  tx_spiral_note: '"Went in circles" = episodes where the agent got stuck (editing the same file again and again, repeated errors, no progress). It wastes tokens — split the task, give clearer context, or give it a way to check its own work.',
...
const ZH: Dict = {
  ...
  tx_episodes_line: '{n} 个回合 · 自主完成率 {a}% · 干预风格 {s} · 原地打转 {sp}',
  ...
  tx_spiral_note: '原地打转 = agent 卡住、在原地兜圈子的回合（反复改同一个文件、连着报错、没有进展）。很费 token——拆小任务、给更明确的上下文、或给它一个能自己检查结果的手段。',
  tx_style_micro: '微操型',
