...
- **空回合排除**：无 token、无工具活动的 episode（用户连发消息、Codex 连续 `turn_context`/aborted turn、文件末尾空 prompt 会开出零活动回合）**不计入** `episode_summary` / `episodes_detail`——否则会稀释 `task_mix`、污染 `autonomy_rate`/`interrupted_rate`（落地见 `EpisodeAccumulator.add`；实测真实数据 219→112 回合）。
