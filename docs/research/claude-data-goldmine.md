...
### 2a. `tipsHistory` —— 本仓库最契合愿景的单一字段

Claude Code 自己记录了**每条官方 tip 向用户展示过多少次**（实测 50+ 条：`plan-mode-for-complex-tasks: 205`、
`custom-agents: 386`、`ultrathink-keyword: 27`…）。与 JSONL 里的真实使用证据交叉，得到**特性曝光 × 采用矩阵**：

| 象限 | 含义 | 建议动作 |
|---|---|---|
| 高曝光 + 已采用 | 工作流成熟 | 表扬，量化收益 |
| **高曝光 + 零采用** | **官方提示了几百次你还没用** | 头号推荐位：这是「官方原生特性」推荐的完美证据源——不猜、不编，直接说「Claude Code 已经提示你 379 次 todo-list，而你从未用过，它能解决你 X 问题」 |
| 零曝光 + 零采用 | 盲区 | 教育性介绍 |
| 低曝光 + 自发采用 | 高级用户信号 | 进画像加分项 |

这正是 ccoach「feature-first、只推官方原生特性」原则的**数据化落地**：推荐不再来自模型的一般知识，
而是来自「官方推过 + 你没接住」的本机证据。探针已验证可行（`docs/research/feature-adoption-probe.mjs`）。
...
- **P0 · CLI 新增 `features` 段**：tipsHistory × 采用证据矩阵（探针已验证）。输出四象限计数 + 「高曝光零采用」清单（特性名白名单）。skill 层把它变成头条建议。
