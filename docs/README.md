...
> **架构方向（规划中，代码尚未实施）**：CLI 将从 Go **迁移到 Node/TypeScript**
> （[ADR 0010](adr/0010-cli-rewrite-node-ccusage.md)），并自建**统一解析层**——因为 ccoach 还要从
> 同一批 JSONL 抓 **user prompt 与习惯指标**（ccusage 数据的超集），所以**向 ccusage 学解析方法、
> 仅作交叉验证、不作运行时依赖**，一个 pass 出「用量 + prompt + 习惯」全量数据
> （[ADR 0013](adr/0013-self-built-unified-parser.md)，取代 0010 D2）；分平台适配器对外吐统一结构，
> 把 **Codex 与 Claude Code 作为对称的一等数据源**，未来扩展到 OpenClaw / Harness 等其它 Agent CLI
> （[ADR 0011](adr/0011-multi-platform-usage-sources.md)）。迁移分步进行、保持 `--json` 契约不变，详见
> [`TODO.md`](TODO.md) T8 / T9。
...
| [0010](adr/0010-cli-rewrite-node-ccusage.md) | CLI 从 Go 迁移到 Node/TS（D2 已被 0013 取代） | 已接受（规划，待实现，取代 0003 的二进制分发） |
...
| [0013](adr/0013-self-built-unified-parser.md) | 自建统一解析层（学 ccusage 方法、仅交叉验证），取代 0010 D2 | 已接受（规划，待实现） |
</content>
