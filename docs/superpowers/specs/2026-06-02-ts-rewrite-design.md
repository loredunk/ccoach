...
> 日期：2026-06-02（2026-06-03 更新进度）· 状态：Phase 1 已落地；**去 Go 完成**；Phase 2（去 Python）进行中——渲染/计算层 + `tools/` 已 `.mjs`/vitest 化，采集类待续
...
- **采集类 Python 并入 ccoach**（🚧 进行中，决策见 [ADR 0018](../../adr/0018-cli-absorbs-collection-prompt-preview.md)，取代「照搬 .mjs」）：
  块 A（行为字段补齐 + skill 的 Claude 行为改吃 `ccoach report --platform claude-code --json`）已落地；
  块 B（`ccoach report --scope`）、块 C（`ccoach sessions` + opt-in 单会话 redacted 预览）待续，完成后删三个 `.py`。
