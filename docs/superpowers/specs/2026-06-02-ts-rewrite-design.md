...
- **采集类 Python 退场**（⏳ 待续）：`collect_claude_behavior.py` / `session_drilldown.py` /
  `claude_session_prompts.py` 改由 `ccoach --json` 取代，或同样手写为 `.mjs`；走 `ccoach` 这条需先给 CLI 加
  scope / 会话钻取 / prompt 预览面（牵涉 `--json` 不含 prompt 原文的隐私契约，值得单开 ADR）。
