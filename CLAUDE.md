...
- `src/` — **TS CLI（Phase 1，当前实现）**：`cli.ts`（cac）/ `index.ts`（`buildReport` + 平台合并）/
  ...
  `pricing.ts`（双平台**离线 fallback** 价表，非权威；权威价走 skill 层联网官方价，ADR 0019）/ `habits.ts` / `prompt-signals.ts` / `text.ts` / `window.ts` / `i18n.ts`（CLI 输出 en/zh 文案表，默认 en，ADR 0026）/
  `feature-adoption.ts`（Claude 特性采用，ADR 0056）/ `codex-feature-adoption.ts`（Codex 特性采用：$CODEX_HOME 本机文件/索引库白名单计数，ADR 0057）/ `emit/{json,text}.ts`。
...
**产品名就是 `ccoach`（代码标识），中英一致、没有中文译名。** 不要再造「(本机) AI 用量教练」这类译名性称呼。

- **称呼产品**：需要描述时用 `ccoach` 作主语 + 功能描述。中文如「本机 ccoach，只读分析 Claude Code / Codex
  用量与习惯」；当句子主语已是 `ccoach`（如 `ccoach = …`）则用纯功能描述「本机用量与习惯分析工具」，别写成
  「ccoach = 本机 ccoach」。英文产品名同样是 `ccoach`；介绍句保留 `a local AI usage coach`（coach 是 ccoach
  的词源、英文是主打市场）是 OK 的，但**可见标题用品牌名**。
- **可见标题用品牌名**：报告 / 成绩卡 / CLI 标题写 `ccoach Report`、`ccoach 报告`、`ccoach Insight 成绩卡`、
  `ccoach Insight Report`、`ccoach 洞察报告`，**不要**「AI 用量(教练)报告 / AI Usage Coach Report」。
- **生成物文件名 = 对应 skill 名**：`ccoach-insight` skill 产出 `ccoach-insight.html`；`ccoach-deepinsight`
  skill 产出 `ccoach-deepinsight.html`。**不要**描述性旧名（`ai-usage-report.html` / `deepinsight-report.html`
  / `codex-report.enriched.html`）。
- **不算译名、可保留**：① 「AI 用量分析 / AI usage report」这类**功能描述**（说的是「分析用量 / 产出一份用量
  报告」，不是把 ccoach 叫教练）；② `ccoach-deepinsight` 作为 coaching skill 自称「深度教练 / deep coach」是它
  的**功能定位**；③ `docs/adr/**`、`docs/superpowers/**` 里的旧措辞 / 旧文件名是**历史事实**，只增不改、不回改。
...
- **工具结果/元数据的「派生信号」是受控例外（ADR 0016/0017）**：`error_signals` / `rework_signals` / `skills` /
  ...
  仅主会话、走去重。项目层另有受控扩展（[ADR 0054](docs/adr/0054-project-file-churn-concentration.md)）：
  跨会话文件级 churn 仅以文件 **basename**（绝不含目录/全路径）聚合计数，只出现在 `--scope project` 的本地
  分析产物，可分享成绩卡绝不引用文件名。
