...
5. Read `/tmp/ai-usage.json`, then write `/tmp/ai-usage-insights.json` following `references/dual-insights-schema.md`.
   ...
   - **Feature adoption (Claude Code only):** `feature_adoption` in the report JSON is official-backed evidence for "native feature you haven't tried" recommendations: `unadopted` comes from Claude Code's own usage counters, and a tip with `still_showing: true` means Claude Code itself still judges that feature unadopted — lead recommendations with these (e.g. `/memory`) before any inferred finding. Honor the `caveats` field: tip watermarks are corroboration only (they drift across versions), and different sources define adoption differently — when they conflict, say so.
   ...
   - **Explain the "why", in plain language.** When recommending file references, say *why* it helps: naming `file_path:line_number` lets the agent jump straight to the right place instead of grepping the whole repo (saves tokens) and cuts "you edited the wrong file" reworks (Anthropic's context guidance: the fuller the context window, the worse quality gets). When the report mentions "episodes" or "spirals", define them in human terms: an episode = one instruction → the agent's work for it; a spiral = an episode where the agent got stuck (same file re-edited / repeated errors / no progress) — flag it as wasteful and suggest splitting the task, giving sharper context, or giving it a way to self-verify. **Use plain words in all prose, both languages** (many readers are not native English speakers): say "went in circles / got stuck" (zh 原地打转) instead of the bare jargon "spiral"; "repeated edits / edited the same file again and again" (zh 反复改动) instead of "churn"; "categories" (zh 分类) instead of "taxonomy". Keep raw field names only in technical margins. When `episode_summary.context_rot` is well-sampled (`low_confidence: false`) and has an `inflection_index`, narrate it as the **context shelf life**: "your sessions degrade after ~N turns — `/clear` or start fresh at task boundaries, push exploration into subagents"; if it is low-confidence, leave it out of the headline entirely.
